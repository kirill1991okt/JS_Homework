...
class AddAndList extends Component {
      ...
      resolve(`
				...
				<div class="tasks">
					<div class="tasks__additional">
						...
						<button class="tasks__btn-clear button" ${
              !tasks.length ? 'disabled' : ''
            }>Clear Tasks List</button>
					</div>
  ...
  setActions() {
    const addTaskTitle = document.getElementsByClassName('task-add__title')[0],
      addTaskDescription = document.getElementsByClassName(
        'task-add__description'
      )[0],
      addTaskBtn = document.getElementsByClassName('task-add__btn-add')[0],
      tasksContainer = document.getElementsByClassName('tasks')[0],
      clearTasksListBtn =
        tasksContainer.getElementsByClassName('tasks__btn-clear')[0],
    ...
    tasksContainer.addEventListener('click', (event) => {
      ...
      switch (true) {
        case targetClassList.contains('tasks__btn-clear'):
          //   const allTask = this.model.getTasksList();
          //   console.log(allTask);
  ...
  getTaskHTML(task) {
    ...
    return `
            <div class="task ${statusDone ? 'task_done' : ''}" data-id="${
      task.id
    }">
                ...
                <div class="task__buttons">
                    ...
                    !statusDone
                      ? `<a class="task__btn-edit button" href="#/task/${task.id}/edit">Edit</a>
                    	 <a class="task__btn-done button" data-id="${task.id}">Done</a>`
                      : ''
                  }
                    <a class="task__btn-remove button" data-id="${
                      task.id
                    }">Remove</a>   
                </div>                            
            </div>
  ...
  clearTasksList(tasksList, clearTasksListBtn) {
    if (confirm('Are you sure?')) {
      this.model.removeTaskList().then(() => {
